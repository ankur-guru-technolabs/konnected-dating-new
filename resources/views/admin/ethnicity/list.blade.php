...
@section('title', 'Ethnicities List')
@section('content')
        ...
        <div class="col-12">
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pb-3 " style="height:50px;padding-top: 0.8rem !important">
                        <h6 class="text-white text-capitalize ps-3">Ethnicities table</h6>
                    </div>
                </div>
                ...
                <div class="card-body px-0 pb-2">
                    <div class="table-responsive p-0 mx-3">
                        <table id="ethnicity_list_table" class="display" style="width:100%">
                            <thead>
                                <tr>
                                    <th>Id</th>
                                    <th>Ethnicity</th>
                            ...
                            <tbody>
                                @foreach($ethnicities as $key=>$ethnicities)
                                <tr>
                                    <td>{{ ++$key }}</td>
                                    <td>{{$ethnicities->name}}</td>
                                    <td>
                                        <a href="" data-bs-toggle="modal" data-bs-target="#editModal" data-id="{{$ethnicities->id}}" data-value="{{$ethnicities->name}}">
                                            <i class="material-icons opacity-10">edit</i>
                                        </a>
                                        <a href="{{route('questions.ethnicity.delete',['id' => $ethnicities->id])}}">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title font-weight-normal" id="addModalLabel">Add Ethnicity</h5>
                        </div>
                        <div class="modal-body">
                            <form role="form text-left" id="addForm"  action="{{ route('questions.ethnicity.store') }}" method="POST">
                                @csrf
            ...
            <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="editModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title font-weight-normal" id="editModalLabel">Update Ethnicity</h5>
                        </div>
                        <div class="modal-body">
                            <form role="form text-left" id="editForm" action="{{ route('questions.ethnicity.update') }}" method="POST">
                                @csrf
...
<script>
    $(document).ready(function() {
        $('#ethnicity_list_table').DataTable();
    });
</script>
@endsection
